...
- A verify pass may raise `damagedImage` but never resolves it: it cannot see a page whose header parses and whose pixels do not. Findings on bytes that have since changed are resolved by the version check regardless.
