...
- **Notifications / Connect.** Add a notification/connect subsystem (Sonarr/Radarr-style) that fires on events — chapter downloaded, download failed, new series/chapter available, import completed, health/error. Support Discord, Telegram, Gotify, and generic webhook/Apprise. Per-connection event toggles + a Test button. Reuse the existing SignalR event pipeline as the event source; store connection config in `AppConfig`, treat any secrets as plaintext credential material like the rest of settings.
