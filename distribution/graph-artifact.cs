...
// Validates an exported edge database and packs it into a publishable artifact.
// Driven by publish-graph.ps1; run directly with:
//   dotnet run distribution/graph-artifact.cs -- <reco|coread> <db> <outDir> <minPairs>
//
// One tool for both graphs because the packing is identical - integrity check, zstd, manifest,
// checksum - and only the table-level validation differs. Two copies would be two copies to keep in
// step, and the half that would drift is the half that decides what is safe to publish.
//
...
if (args.Length < 4)
{
    Console.WriteLine("usage: graph-artifact.cs <reco|coread> <db> <outDir> <minPairs>");
    return 2;
}

var kind = args[0].Trim().ToLowerInvariant();
if (kind is not ("reco" or "coread"))
{
    Console.WriteLine($"error: unknown graph '{args[0]}' (expected reco or coread)");
    return 2;
}

var dbPath = args[1];
var outDir = args[2];
var minPairs = long.Parse(args[3], CultureInfo.InvariantCulture);
...
var baseName = kind == "reco" ? "reco-edges" : "coread-edges";
...
Console.WriteLine($"pairs      : {pairs} over {series} series");
Console.WriteLine(kind == "reco"
    ? $"providers  : {providers} (anilist {aniListPairs}, mal {malPairs}, {reciprocal} reciprocal)"
    : $"built from : {users} readers");
