...
export interface Analysis { status: string; hash?: string; pages: {name: string}[]; problems: {kind: string; severity: string; message: string}[]; repetitions: {first: number; second: number; kind: string; distance: number}[] }
export interface FileDetail { file: HealthFile; analysis: Analysis; findings: HealthFinding[]; chapters: {id: number; title?: string; number?: number; wanted: boolean}[]; mappings: {id: number; sourceName: string}[] }
