...
/// <c>mangarr.db</c> plus <c>config.json</c> (credential material) and a manifest. Big/regenerable
...
{
    private const string DbEntry = "mangarr.db";
    ...
    public async Task<BackupInfo> CreateAsync(string kind, CancellationToken ct)
    {
        ...
        var name = $"mangarr-{createdUtc:yyyyMMdd-HHmmss}-{kind}.zip";
    ...
    public async Task PruneAsync(CancellationToken ct)
    {
        ...
        var byKind = Directory.EnumerateFiles(paths.BackupDir, "mangarr-*-*.zip")
    ...
    private async Task StageAsync(Stream zipStream, CancellationToken ct)
    {
        ...
        var dbEntry = archive.GetEntry(DbEntry)
            ?? throw new InvalidOperationException("Backup is missing mangarr.db — not a Mangarr backup.");
        ...
        if (manifest?.LastMigration is { } last)
        {
            ...
            if (!known.Contains(last))
                throw new InvalidOperationException(
                    ...
                    "Upgrade Mangarr before restoring it.");
        }
