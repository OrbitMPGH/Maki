...
public class KavitaClient(IHttpClientFactory httpClientFactory)
{
    ...
    /// <summary>The series' web links (comma-separated in Kavita's metadata; Mangarr pushes these).</summary>
    ...
    private async Task<string> GetTokenAsync(string baseUrl, string apiKey, bool force, CancellationToken ct)
    {
        ...
        try
        {
            ...
            var response = await client.PostAsync(
                $"api/Plugin/authenticate?apiKey={Uri.EscapeDataString(apiKey)}&pluginName=Mangarr", null, ct);
            response.EnsureSuccessStatusCode();
