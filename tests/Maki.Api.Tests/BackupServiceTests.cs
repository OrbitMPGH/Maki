...
public class BackupServiceTests : IDisposable
{
    ...
    {
        _configDir = Path.Combine(Path.GetTempPath(), "maki-backup-tests", Guid.NewGuid().ToString("N"));
    ...
    {
        Assert.Throws<FileNotFoundException>(() => Build().PathFor("maki-20260101-000000-manual.zip"));
    }
    ...
    public async Task Prune_keeps_the_newest_per_kind()
    {
        ...
        {
            await File.WriteAllTextAsync(Path.Combine(_paths.BackupDir, $"maki-{stamp}-auto.zip"), "x");
        }
        await File.WriteAllTextAsync(Path.Combine(_paths.BackupDir, "maki-20260101-000000-manual.zip"), "x");
        ...
        Assert.Contains("maki-20260104-000000-auto.zip", remaining);
        Assert.Contains("maki-20260103-000000-auto.zip", remaining);
        Assert.Contains("maki-20260101-000000-manual.zip", remaining);
        Assert.DoesNotContain("maki-20260101-000000-auto.zip", remaining);
    }
