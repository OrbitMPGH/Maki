...
/// exact normalized match, original-title fallback, subtitle-prefix acceptance, and the
/// guards that leave a series unmapped (no match, already mapped, source error).
/// </summary>
public class SourceMatchServiceTests : IDisposable
{
    ...
    [Fact]
    public async Task Subtitle_prefix_variant_is_accepted()
    {
        var seriesId = _db.SeedSeries("Hajime no Ippo");
